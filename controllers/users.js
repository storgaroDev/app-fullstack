...
userRouter.post('/', async (request, response) =>{
    const {name, email, password, usuario} = request.body;
    ...
    //
    if (!(name && email && password && usuario)) {
  ...
  //                           <td>
  //                             <p>Hi there,</p>
  //                             <p>Thanks for getting started with the Contact App! <br>
  ...
  //                                       <tbody>
  //                                         <tr>
  //                                           <td> <a href="${PAGE_URL}verify/${savedUser.id}/${verifyToken}" target="_blank">Verify email</a> <br>
  //                                           </td>
  //                                         </tr>
  //                                       </tbody>
  //                                     </table>
...
userRouter.patch('/:id/:token', async (request, response) =>{
  ...
  //Buscar si el email ya esta regsitrado
  const user = await User.findById(request.params.id, { checked }, { new: true });
  ...
  await User.findByIdAndUpdate(request.params.id, { name, email });
...
userRouter.get('/', async (request, response) =>{
  ...
  if (user.role !== 'admin') {
    return response.status(401).json({isAdmin: false});
  }
